<template>
  <div class="main-unit-wide main-visual">
    ...
    <Swiper
      :options="{ 
        ...
        on: {
          ...
          touchEnd : sliderMv, // 230209 상단배너 : 추가
        },
        pagination: {
    ...
    >
      ...
      <button slot="button-prev" class="swiper-button-prev mv-prev" @click="swToMv"></button>
      <button slot="button-next" class="swiper-button-next mv-next" @click="swToMv"></button>
    </Swiper>
    <!-- eslint-disable -->
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    stopPlayingVideo(){
      const me = this
      me.$el.querySelectorAll(".mv-slide-test3 .video-player:not(.paused)").forEach(function(el){
        try{
    ...
    playActiveVideo(){
      const me = this
      try{
        const el = me.$el.querySelector(".mv-slide-test3.swiper-slide-active .video-player.paused")
        if(el){
          el.__vue__.playVideo()
        }
      }catch(e){}
    },
    swToMv(){  // 230209 상단배너 : 추가
      const me = this
      const prvIdx = me.swiper.realIndex
      const max = me.data.list.length - 1
      setTimeout(()=>{
        const idx = me.swiper.realIndex
        if(prvIdx===idx && (idx===max || idx===0)) me.slideHide = true
        
        if(idx===max && prvIdx===idx) me.swiper.slideTo(0)
        else if(idx===0 && prvIdx===idx) me.swiper.slideTo(max)
        setTimeout(()=>{
          me.slideHide = false
        },300)
      },100)
    },
    sliderMv(){ // 230209 상단배너 : 추가
      const me = this
      const b = me.swiper.isBeginning
      const e = me.swiper.isEnd
      const d = me.swiper.swipeDirection

      if(b && d==='prev') me.swToMv()
      else if(e && d==='next') me.swToMv()
    },
  },// methods
}
</script>
