<template>
<v-navigation-drawer
  v-model="filterLayerVisible" fixed temporary width="500px" overlay-color="#000" overlay-opacity="0.5" class="filter-layer"
  :class="[
    {
...
>
  ...
  <div class="filter-main-page">
    <div class="filter-layer-head">
      <button class="filter-close-btn" @click="closeFilterLayer"><span class="invisible">닫기</span></button>
    </div>
    <!-- [퍼블수정]24.08.27 검색 솔루션 -->
    <div class="filter-layer-body">
      <ul>
        <!-- 선택된 필터 -->
        <li v-if="selectedList.length > 0" class="filter-list-item2">
          <div class="filter-list-title">선택된 필터</div>
          <div class="filter-list-cont">
            <ul class="selected-filters">
              <li v-for="item, idx in selectedList" :key="idx">
                ...
                <span v-if="item.filter.type==='size'" class="sizePrefix">{{item.parent.label}}</span>
                {{item.label}}
                <button @click="selectFilter(item.filter, item, 'delete')"><span class="invisible">삭제</span></button>
              </li>
            </ul>
          </div>
        </li>
        <!-- 선택된 필터 -->
        <!-- 상품정보 -->
        <li class="filter-list-item2">
          <div class="filter-list-title">상품정보</div>
          <div class="filter-list-cont">
            <v-checkbox
                v-model="productInfo.saleProduct"
                label="SALE상품"
                class="cmp-form-checkbox"
              ></v-checkbox>
            <!-- <v-checkbox
                v-model="productInfo.exclusiveProduct"
                label="EXCLUSIVE 상품"
                class="cmp-form-checkbox"
              ></v-checkbox> -->
            <v-checkbox
                v-model="productInfo.excludeSoldOut"
                label="품절상품 제외"
                class="cmp-form-checkbox"
              ></v-checkbox>
            <!-- <v-checkbox
                v-model="productInfo.handsomeDelivery"
                label="한섬딜리버리"
                class="cmp-form-checkbox"
              ></v-checkbox> -->
          </div>
        ...
        <!-- 상품정보 -->
        <!-- 필터 리스트 (카테고리, 브랜드,...) -->
        <li v-for="item, idx in filterList" :key="idx" class="filter-list-item">
          <button class="filter-list-btn" @click="showSubPage(item)">
            {{item.label}}
            <span v-if="item.selectedItems.length > 0 && item.type !== 'price'">{{item.selectedItems.length}}</span>
          </button>
        </li>
        <!-- 필터 리스트 (카테고리, 브랜드,...) -->
        <!-- 재검색 -->
        <li v-if="filterResearchVisible" class="filter-list-item2 mt-30">
          <div class="filter-list-title">결과 내 재검색</div>
          <div class="reserch-cover">
            <v-radio-group v-model="rodioMode1" row class="cmp-form-radioGroup">
              <v-radio label="포함" class="cmp-form-radio" value="1"></v-radio>
              <v-radio label="제외" class="cmp-form-radio" value="2"></v-radio>
            </v-radio-group>
          </div>
          <div class="filter-list-cont serch-filter">
            <div class="cmp-form-searchBox search-type">
              <v-text-field label="검색어를 입력해주세요" clearable height="48" class="cmp-form-input"></v-text-field>
              <v-btn icon><i class="cmp-icon cmp-icon-32 i-search2">검색</i></v-btn>
            </div>
          </div>
          <div class="filter-list-cont">
            <ul class="selected-filters">
              <li  v-for="sel2, sdx2 in serchSlides" :key="sdx2">
                <template v-if="sel2.addfilter === true">포함-{{sel2.label}}</template>
                <template v-else>제외-{{sel2.label}}</template>
                <button><span class="invisible">삭제</span></button>
              </li>
            </ul>
          </div>
        </li>
  ...
  <div class="filter-sub-page">
    <div class="filter-layer-head">{{currentFilter.label}} 선택</div>
    <div class="filter-sub-cont" :class="currentFilter.type+'-list'">
      ...
      <ul>
        <!-- [퍼블수정]24.08.27 검색 솔루션 -->
        <!-- 가격 직접입력 -->
        <li v-if="currentFilter.type==='price'" class="depth1 pb-20 mb-3" :class="{ 'selected' : currentFilter.customInput }">
          <div class="price-slider">
            <button @click.stop="selectCustomPrice">
              <div class="label"><em class="radio"></em> 직접입력</div>
            </button>
            <div class="slider-wrap">
              <div class="slider">
                <div class="line"></div>
                <div class="handle"></div>
                <div class="handle right" style="left:100%"></div>
                <button class="box" @mousedown="priceDragStart"></button>
              </div>
            </div>
            <div class="prices">
              <label for="customPrice1">
                <input id="customPrice1" v-model="price1" type="tel" :min="minPrice" :max="maxPrice" @keyup="priceKeyUp" @blur="priceKeyUp" @focus="priceKeyUp" />
                <span>{{price1 | number}}</span>
              </label>
              <label for="customPrice2">
                <input id="customPrice2" v-model="price2" type="tel" :min="minPrice" :max="maxPrice" @keyup="priceKeyUp" @blur="priceKeyUp" @focus="priceKeyUp" />
                <span>{{price2 | number}}</span>
              </label>
            </div>
          </div>
        </li>
  ...
  <!-- [퍼블수정]24.08.27 검색 솔루션  -->
  <div class="popup-fix-mySize" v-if="showMySize && currentFilter.type==='size'">
...
export default {
  ...
  created(){
    const me = this
    EventBus.$on("showFilterLayer", me.showFilterLayer)
    // EventBus.$on("showFilterResearch", me.showFilterResearch)
    EventBus.$on("filterLayerReset", me.filterResetListener)
    EventBus.$on("filterLayerDelete", me.filterDeleteListener)
  },
  methods:{
    ...
    // 필터 레이어 열기/닫기
    showFilterLayer(vis, type){
      const me = this
      // me.filterLayerType = type
      me.filterResearchVisible = (type === "search")
      me.filterLayerVisible = (vis === true)
      if(me.filterLayerVisible){
        me.loadData()
      }
    },
    closeFilterLayer(){
