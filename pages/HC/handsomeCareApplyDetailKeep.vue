      ...
      <div class="my-page-r-box hansomecare-application">
        ...
        <div class="section-wrapper">
          ...
          <div v-else class="cmp-order-state">
            ...
            <div v-if="stepActive == '4' && !stepKeep" class="text-center"> 
              <p class="cmp-font fnt-20 mt-10">상품 세탁이 진행중입니다.</p>
            </div>
            <div v-if="stepActive == '4' && stepKeep" class="text-center"> 
              <p class="cmp-font fnt-20 mt-10">세탁이 완료되어</p>
              <v-layout row align-center justify-center>
                <p class="cmp-font fnt-20">0000년 00월 00일까지 상품 보관 예정입니다.</p>
                <CommonTooltipBox right class="ml-4">
                  <template #content>
                    <ul class="cmp-list list-dotType text-left">
                      <li>배송지 및 보관기간 변경을 원하실 경우,한섬 크리닝 컨시어지(031-3888-0260)으로 연락 부탁드립니다.</li>
                      <li>선택하신 날짜가 주말/공휴일일 경우 실제 수령일과 차이가 발생할 수 있습니다.</li>
                    </ul>
                  </template>
                </CommonTooltipBox>
              </v-layout>
              <!-- [퍼블수정]23.01.11 달력 추가 -->
              <button class="cmp-font fnt-13 button mt-6" @click="dateselectBtn"><span class="line">보관종료일 변경</span></button>
              <div class="cmp-form-calendarBox btn-type">
                <div class="calendar-box">
                  <v-menu ref="dateDialog" v-model="dateModal" :flat="true" :close-on-content-click="false" offset-y content-class="cmp-calendar-layer">
                    <template #activator="{ on, attrs }">
                      <v-text-field v-model="dates" outlined readonly class="cmp-form-input label-hide" v-bind="attrs" v-on="on"></v-text-field>
                    </template>
                    <v-date-picker v-model="dates" no-title :flat="true" scrollable class="cmp-form-calendar" :day-format="getDate" @input="dateSelect"></v-date-picker>
                  </v-menu>
                </div>
              </div>
              <!-- // [퍼블수정]23.01.11 달력 추가 -->
          ...
          <!--//세탁상품-->
          <!-- '접수'에만 노출 --> <!-- [퍼블리싱]240110 케어플러스 세탁 THE STAR 전용 보관 서비스 - '보관중'에도 함께 노출됨 -->
          <template v-if="stepActive == '1' || stepKeep">
          <!-- 상품 수거 및 수령방식-->
