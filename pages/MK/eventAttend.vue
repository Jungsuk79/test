<template>
  <div class="contents"><!-- 레이아웃 설명 /ko/guide/Structure 참고 -->
    <div class="content-wrap">
      <div class="event-detail detail-attend">
        <div class="e-title">
        ...
        <div class="event-inner">
          <div class="date-area">
            <v-sheet class="calendar">
              ...
              >
                <template #day="{ past, date }">
                  <span class="day">{{calculatedDate(date)}}</span>
                  <template v-if="past && attended.indexOf(date) > -1">
                    <div class="attended">
                      <v-img src="/images/common/icon-check-white.svg" alt=""></v-img>
                    </div>
                  </template>
                </template>
              </v-calendar>
            ...
            <div class="right">
              <div class="upper">
                <div class="now-month">
                  <strong>{{new Date().getMonth()+1 }}</strong>
                  <span>월</span>
                </div>
                <div class="now-total">
                  총 <strong>{{attended.length}}</strong>회 출석
                </div>
              </div>              
              <v-btn nuxt color="primary" large class="mt-34">출석하기</v-btn>
              <!-- <v-btn nuxt color="primary" large disabled class="mt-34">출석하기</v-btn> --> <!-- 출석완료시 -->
            </div>
          </div>
...
export default {
  ...
  created() {
    ...
    this.today = today;
    this.attended.push(yesterday.toISOString().split("T")[0]);    
  },
  methods: {
    calculatedDate(date) {
