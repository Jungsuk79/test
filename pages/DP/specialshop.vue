<template>
  <div class="contents"><!-- 레이아웃 설명 /ko/guide/Structure 참고 -->  
    <div class="content-wrap">
      <ul class="brand-specialshop-tab">
        <li v-for="(item, index) in tabMenu" :key="index" :class="{on:index===tabIdx}">
          <button @click="tabIdx = index"><!-- [퍼블수정]23.01.30  -->
            <span>{{item}}</span>
            <span>{{item}}</span>
          </button>
        </li>
      </ul>
      <div class="brand-specialshop-visual">
        <v-img src="https://img.thehandsome.com/md/publish/dummy/pc/brand-special-shop-img.png" alt="COVER PRESENTAION 대표 이미지"></v-img>
      </div>
...
export default {
  ...
  data: () => ({
    ...
    tabMenu : [
      'COVER PRESENTAION',
      'KNOTTED UNEXPECTED WAY EDITION 01',
      'EXPERT NEW WAY - TO GO',
      'LIFE MIX LAYER',
      'PALETTE',
    ],
    filterData: { list:[] },
  ...
  mounted(){
    EventBus.$emit("setHeader", {static: true, type: "brand", brand: "SJSJ", menu :"SPECIAL SHOP"})
  },// mounted
