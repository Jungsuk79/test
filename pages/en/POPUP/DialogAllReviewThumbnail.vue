          ...
          <div v-for="(item,index) in reviewList" :key="index" class="thumbnail-list">
            <div role="button" tabindex="0" class="thumbnail-btn" @click="reviewDetailOpen">
              <div class="btn-cnt" :class="{on:index===viewIdx}">
                <v-img v-if="item.type==='image'" :src="item.image" alt="review thumbnail" />
...
export default {
  ...
  created(){
    const me = this
    if(me.viewIdx!==null) me.popHeaderTitle = 'View Collection of Photos/Videos <strong>'+(me.viewIdx+1)+'</strong>/'+me.reviewList.length
    else me.popHeaderTitle = 'View Collection of Photos/Videos '+me.reviewList.length
  },
  methods: {
    closeDialog() {
      this.$emit('close')
