    ...
    <div class="content-wrap search-content-wrap">
      ...
      <div class="search-result-top">
        ...
        <div class="cmp-form-searchBox search-type star-type">
          <v-text-field v-model="inputText" @input="checkInput" label="키워드를 입력해주세요" clearable height="48" class="cmp-form-input label-hide"></v-text-field>
        ...
        <div class="style-filter-btn">
          <v-btn outlined large height="46" class="width-160"><i class="cmp-icon cmp-icon-18 i-filter"></i><span>Style Filter</span></v-btn>
          <p class="txt">If you want to see products of various styles, use the style filter search!</p>
        </div>
        <!-- //스타일필터 -->
      </div>
      <!-- //검색결과 상단 -->
      ...
      <div v-if="!testSearchNone" class="cmp-tabs box-type search-tab">
        <v-tabs v-model="tabs" fixed-tabs>
          <v-tab>shopping <template v-if="testSearchNoneShopping">0</template><template v-else>2,268</template></v-tab>
          <v-tab>Review <template v-if="testSearchNoneReviw">0</template><template v-else>1,321</template></v-tab>
          <v-tab>Contents <template v-if="testSearchNoneContents">0</template><template v-else>1,143</template></v-tab>
        </v-tabs>
      </div>
      <!-- //쇼핑,컨텐츠 tab -->
      <!-- [퍼블수정]24.08.27 검색 솔루션 -->
              ...
              <div class="filter-sort-wrap serch-var">
                <div class="filter-sort-btns split-layout">
                  <div class="filter-button">
                    <button class="filter-icn" @click="showFilter">Filter</button>
                    <button @click="showqkFilterLayer('category')">category 4</button>
                    <button @click="showqkFilterLayer('brand')">brand</button>
                    <button @click="showqkFilterLayer('color')">color 1</button>
                    <button @click="showqkFilterLayer('size')">size</button>
                  </div>
                  <!-- [퍼블수정]공통-우측 필터 수정 -->
                  ...
                  <SwiperSlide v-for="(item, index) in planshopList.list" :key="index">
                    <div class="swiper-unit">
                      <nuxt-link :to="item.link">
                        ...
                        <span class="txt bold">{{item.brand_name}}</span>
                      </nuxt-link>
                    </div>
                  </SwiperSlide>
                </Swiper>
  ...
  data() {
    return {
      ...
      unitReview : [ // 리뷰 리스트
        {
          data : "2022.02.06",
          prd_name: 'lamb wool blend cape muffler',
          // 글로벌 (K-Review) 케이스 추가
          userInfo : ["COCOA", "M", "THE HANDSOME.COM", "(K-Review)"],
          attachFiles: [
            {
          ...
          reviewScoreStar: 5,
          reviewTxt : "It's not soft, but it seems really useful. The buttons are a godsend. It can be used as a shawl or muffler, so it seems like it would be a good winter item. I think it would be better if the price ", // 리뷰내용
        },
        {
          helped_point: '100',
          data : "2022.02.06",
          ...
          reviewScoreStar: 3,
          reviewTxt : "It's not soft, but it seems really useful. The buttons are a godsend. It can be used as a shawl or muffler, so it seems like it would be a good winter item. I think it would be better if the price wa...It's not soft, but it seems really useful. The buttons are a godsend. It can be used as a shawl or muffler, so it seems like it would be a good winter item. I think it would be better if the price wa...It's not soft, but it seems really useful. The buttons are a godsend. It can be used as a shawl or muffler, so it seems like it would be a good winter item. I think it would be better if the price wa...It's not soft, but it seems really useful. The buttons are a godsend. It can be used as a shawl or muffler, so it seems like it would be a good winter item. I think it would be better if the price ", // 리뷰내용
        },
        {
          data : "2022.02.06",
          prd_name: 'lamb wool blend cape muffler',
