   ...
   class="popup-enterShipping" 
    ...
    <template #body>
      ...
      <v-layout row class="cmp-form-chkGroup wide left">
        <v-checkbox label="내 정보 불러오기" class="cmp-form-checkbox"></v-checkbox>
      </v-layout>
      <!-- //카테고리 필터 -->
      <div class="cmp-form-boxGroup">
        ...
        <div class="cmp-form-box required">
          ...
          <v-text-field v-model="model1" outlined type="text" value="회사주소지" label="배송지명을 입력해 주세요." clearable class="cmp-form-input"></v-text-field>
        </div>
        <!-- //배송지명 -->
        <!-- 수령인 -->
        <div class="cmp-form-box required">
        ...
        <div class="cmp-form-box pb-4" :class="{valid:formBoxErrorTest}">
          ...
          <CommonSelectBox id="selectId5" title="도메인 선택" @submit="deliveryMsgInputSet">
            <template #optionList>
              <option value="placeholder">배송메세지 입력 안함</option>
              <option value="">배송메세지 입력 안함</option>
      ...
      <div class="popup-btn-box">
        <v-btn large block color="primary">등록하기</v-btn> 
        <!-- 배송지 수정 시 노출
        <v-btn outlined large color="primary">삭제하기</v-btn>
        <v-btn large block color="primary">수정하기</v-btn> 
        -->
      </div>
    </template>
  </DialogBasic>
